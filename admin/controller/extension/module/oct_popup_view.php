...
class ControllerExtensionModuleOctPopupView extends Controller {
    ...
    public function index() {
        ...
        $data['action'] = $this->url->link('extension/module/oct_popup_view', 'token=' . $this->session->data['token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/module/oct_popup_view.tpl', $data));
    }
    ...
    public function install() {
        ...
        $this->model_user_user_group->addPermission($this->user->getId(), 'access', 'extension/module/oct_popup_view');
        $this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'extension/module/oct_popup_view');
        
        $this->model_setting_setting->editSetting('oct_popup_view', array(
            'oct_popup_view_data' => array(
                'status' => '1',
                'quantity' => '1',
                'specification' => '1',
                'review' => '1',
        ...
        if (!in_array('oct_popup_view', $this->model_extension_extension->getInstalled('module'))) {
            $this->model_extension_extension->install('module', 'oct_popup_view');
        }
    ...
    public function uninstall() {
        $this->load->model('extension/extension');
        $this->load->model('setting/setting');
        $this->model_extension_extension->uninstall('module', 'oct_popup_view');
        $this->model_setting_setting->deleteSetting('oct_popup_view_data');
    }
    
    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/oct_popup_view')) {
