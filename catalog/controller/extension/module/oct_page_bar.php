...
class ControllerExtensionModuleOctPageBar extends Controller {
  public function index() {
    ...
    $data['oct_page_bar_data'] = $oct_page_bar_data = $this->config->get('oct_page_bar_data');

//    $limit = (isset($oct_page_bar_data['limit'])) ? $oct_page_bar_data['limit'] : 20;
//
//    $data['total_viewed'] = isset($this->session->data['oct_product_viewed']) ? ((count(array_unique($this->session->data['oct_product_viewed'])) <= $limit) ? count(array_unique($this->session->data['oct_product_viewed'])) : $limit) : 0;
//    if ($this->customer->isLogged()) {
//      $data['total_wishlist'] = ($this->model_account_wishlist->getTotalWishlist() <= $limit) ? $this->model_account_wishlist->getTotalWishlist() : $limit;
//    } else {
//      $data['total_wishlist'] = isset($this->session->data['wishlist']) ? ((count($this->session->data['wishlist']) <= $limit) ? count($this->session->data['wishlist']) : $limit) : 0;
//    }
//    $data['total_compare'] = isset($this->session->data['compare']) ? ((count($this->session->data['compare']) <= $limit) ? count($this->session->data['compare']) : $limit) : 0;
//    $data['total_cart']    = ($this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0) <= $limit) ? $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0) : $limit;
