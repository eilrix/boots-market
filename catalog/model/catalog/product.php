...
class ModelCatalogProduct extends Model {
	...
	public function getCategoriesInfo($product_id) {
		$query_cat_id = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "'");

        //$query_name = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_description WHERE category_id = '" . (int)$product_id . "'");

        $query_cat_id_arr = $query_cat_id->rows;
        $data = array();

        foreach ($query_cat_id_arr as $category) {
            $category_id = $category['category_id'];

            $query_name = $this->db->query("SELECT meta_h1 FROM " . DB_PREFIX . "category_description WHERE category_id = '" . (int)$category_id . "'");
            $query_name = $query_name->rows[0];
            $query_name_str = $query_name['meta_h1'];


            $cat_url = "category_id=" . $category_id;
            $query_seo_url = $this->db->query("SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = '" . $cat_url . "'");

            $query_seo_url = $query_seo_url->rows[0];
            $query_seo_url_str = $query_seo_url['keyword'];

            $data[] = array(
                'id' => $category_id,
                'name' => $query_name_str,
                'url' => $query_seo_url_str
            ) ;

        }

        //return $query_cat_id_arr;
        return $data;
        //return "SELECT * FROM " . DB_PREFIX . "oc_category_description WHERE category_id = '" . (int)$product_id . "'";
	}
