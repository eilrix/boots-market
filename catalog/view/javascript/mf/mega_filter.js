	...
	*/
	...
	initResponsive: function(){
		...
		if( self._box.hasClass( 'mfilter-hide-container' ) ) {
            ...
            //column = jQuery('#content .row.sort-row');
		...
		var id = 'mfilter-free-container-' + jQuery('[id^="mfilter-free-container"]').length,
			...
			cnt, cnt2, cnt3, btn, closeBtn, closeBCKGR;
		...
		if( ! jQuery( '.mfilter-free-container' ).length ) {
			...
			btn = jQuery('<div class="mfilter-free-button filter-vector-ico-black-fa">')
				.appendTo( cnt )
				.click(function(){
					...
					if( hidden ) {
						cnt.animate(self._options.direction == 'rtl' ? {
                            ...
                            closeBtn.removeClass('mfilter-hide-btn');
                            closeBCKGR.removeClass('mfilter-hide-btn');
						...
						cnt.animate(self._options.direction == 'rtl' ? {
                            ...
                            closeBtn.addClass('mfilter-hide-btn');
                            closeBCKGR.addClass('mfilter-hide-btn');
                            $("#oct-bluring-box").prop("onclick", null);
            ...
            closeBtn = jQuery('<div class="mfilter-close-free-button mfilter-hide-btn">×</div>').appendTo( cnt ).click(function(){
                	btn.click();
			});
            closeBCKGR = jQuery('<div class="mfilter-close-background mfilter-hide-btn"></div>').appendTo( cnt ).click(function(){
                btn.click();
            });
	...
	__initLoader: function() {
		var color = this.hexToRgb(typeof this._options.color == 'undefined' ? '#000' : this._options.color.loader_over_results);
		
		this._jqLoader = jQuery('<span class="mfilter-ajax-loader-container" style="cursor: wait; z-index: 100; margin: 0; padding: 0; position: absolute; text-align: center; background-color: rgba(0,0,0,0.4);"></span><script type="text/javascript">/*setTimeout( ()=>{window.pageHandler.reloadShowMoreAfterFilter();}, 2500);*/</script>')
			.prependTo( this._jqContent )
			.html( '<img src="/catalog/view/theme/oct_techstore/image/ring-alt.svg" alt="" />' )
			.hide();
	},
	...
	_initDisplayItems: function() {
		...
		self._box.find('.mfilter-filter-item').each(function(i){
            ...
            self._content_list.push(content)
	...
	_initHeading: function() {
		...
		self._box.find('.mfilter-heading').click(function(){
			...
			if( jQuery(this).hasClass('mfilter-collapsed') ) {
				//Open
				//Collapse all other opts
				//////////////
				/*
                self._content_list.forEach(function(opts_cont) {
                    jQuery(opts_cont).slideUp('normal', function(){
                        if( self._relativeScroll != null )
                            self._relativeScroll.refresh();
                    });
                    var header_title = jQuery(opts_cont).parent().find('> .mfilter-heading');
                    header_title.addClass('mfilter-collapsed');
                    jQuery(opts_cont).focusout(function() {

                    });
                });

                $price_opts = $('#mfilter-box-1 .mfilter-price .mfilter-content-opts');
                $price_opts.slideUp('normal', function(){
                    if( self._relativeScroll != null )
                        self._relativeScroll.refresh();
                });
                $price_head = $('#mfilter-box-1 .mfilter-price .mfilter-heading');
                $price_head.addClass('mfilter-collapsed');
                */
				////////////////
