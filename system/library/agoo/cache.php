...
class agooCache extends Controller
{
	...
	public function agooconstruct($settings) {
    	...
    	if (isset($settings['cache_max_hache_folders_level']) && $settings['cache_max_hache_folders_level'] != '' && $settings['cache_max_hache_folders_level'] > 0 && $settings['cache_max_hache_folders_level'] < 10) {
        	$this->max_hache_folders_level = $settings['cache_max_hache_folders_level'];
    	}
		...
		if (!$this->config->get('blog_work') || $pieces_array[0] != 'blog') {
			...
			if (isset($pieces_array[0]) && $pieces_array[0] == 'blog') {
				...
				if ($pieces != 'db') {
                    ...
                    if ($hache != $pieces) {						$folder_hache = substr($hache, 0, $this->max_hache_folders_level);
                    	$pieces = $pieces.'/'.$folder_hache;
                    }
